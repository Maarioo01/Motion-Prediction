...
if __name__ == "__main__":
    ...
    try:
        ...
        # Some decoder heads aren't used on every forward pass (occlusion vs. observed
        # branches), which trips plain DDP's gradient-sync assumptions across multiple
        # GPUs - only relevant when actually running on >1 device.
        _devices = trainer_config["devices"]
        _multi_gpu = isinstance(_devices, (list, tuple)) and len(_devices) > 1
        trainer = Trainer(
                          ...
                          strategy="ddp_find_unused_parameters_true" if _multi_gpu else "auto",
