...
| [TrajFlow](https://github.com/DSL-Lab/TrajFlow) | 5-6 | ✅ working, full-scale preprocessing done | `/raid/waymo/scenario` (all splits present incl. plain `training`); all 5 splits preprocessed successfully, see [`TRAINING_PLAN.md`](docs/TRAINING_PLAN.md) | ❌ | MIT |
...
| [MTR](https://github.com/sshaoshuai/MTR) | — (not in original survey shortlist, added later) | ✅ working, full-scale preprocessing done | `/raid/waymo/scenario` (present, plain `training`/`validation`); preprocessed successfully after fixing two real upstream bugs, see [`TRAINING_PLAN.md`](docs/TRAINING_PLAN.md) | ❌ | Apache-2.0 |
